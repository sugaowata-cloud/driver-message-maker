(() => {
  ...
  const MAIN_PLACES = ['指定場所', 'ラトゥール', 'ラトゥール下', '伊丹空港駐車場', '伊丹空港ゲート前'];
  const STANDARD_PLACES = { pickup: MAIN_PLACES, waiting: MAIN_PLACES, meeting: MAIN_PLACES, arrival: MAIN_PLACES };

  // 文面テンプレートの基本設定。表現を変える場合はここを編集します。
  const MESSAGE_TEMPLATES = {
    pickupConfirm: { title: '了解しました・○時に向かいます（確認）', icon: '✅', opening: '承知しました！', closing: 'よろしくお願いします。' },
  ...
  function placeField(name, label, category, value = '', placeholder = '') {
    ...
    const selected = places.includes(value) && value !== '指定場所' ? value : '指定場所';
    const customValue = selected === '指定場所' ? value : '';
    return `<div class="field"><label for="${name}Choice">${label}</label><select id="${name}Choice" name="${name}Choice" data-place-choice="${name}">${optionHtml(places, selected)}</select></div><div id="${name}CustomWrap" class="field${selected === '指定場所' ? '' : ' hidden'}"><label for="${name}">指定場所を入力</label><input id="${name}" name="${name}" type="text" value="${escapeHtml(customValue)}" placeholder="${escapeHtml(placeholder)}" autocomplete="off"></div>`;
  }
  ...
  function renderHome() {
    renderComposer('pickupNotice', {}, true);
  }
  ...
  function handleHistoryAction(event) {
    ...
    if (event.target.closest('.history-copy')) { copyText(item.message); return; }
  ...
  function renderComposer(type, values = {}, isHome = false) {
    currentType = type; currentInputs = clone(values || {}); setView(isHome ? 'home' : 'composer'); useTemplate('composerTemplate');
    const template = MESSAGE_TEMPLATES[type];
    document.getElementById('composerIcon').textContent = '✉️';
    document.getElementById('composerHeading').textContent = '連絡文を作成';
    document.getElementById('situation').innerHTML = SITUATIONS.map((item) => `<option value="${item.value}"${item.value === type ? ' selected' : ''}>${escapeHtml(item.label)}</option>`).join('');
    ...
    form.elements.situation.addEventListener('change', () => {
      renderComposer(form.elements.situation.value, { opening: form.elements.opening.value, closing: form.elements.closing.value }, isHome);
    });
  ...
  function buildFields(type, v) {
    ...
    if (type === 'dinnerWaiting') return `<section class="form-card"><h3>会食中の待機</h3>${selectField('dinnerWaitPlan','待機方法',['このまま付近で待機しておきます。','付近での待機が困難なため、別の場所で待機します。','付近の駐車場にて待機します。','離れた場所で待機し、○時ごろ付近に戻ります。'],v.dinnerWaitPlan || 'このまま付近で待機しておきます。')}${placeField('dinnerWaitPlace','待機場所・駐車場名（任意）','waiting',v.dinnerWaitPlace,'例：近隣のコインパーキング')}<div id="dinnerReturnTimeWrap" class="hidden">${timeField('dinnerReturnTime','付近に戻る時刻（目安）',v.dinnerReturnTime)}</div>${textareaField('note','補足','文面に加えたい内容',v.note)}</section>`;
  ...
  function handleConditionalFields() {
    ...
    toggle('dinnerReturnTimeWrap', form.elements.dinnerWaitPlan?.value === '離れた場所で待機し、○時ごろ付近に戻ります。');
    form.querySelectorAll('[data-place-choice]').forEach((select) => toggle(`${select.dataset.placeChoice}CustomWrap`, select.value === '指定場所'));
  }
  function formValues(form) {
    ...
    Object.keys(values).filter((key) => key.endsWith('PlaceChoice')).forEach((key) => {
      const base = key.slice(0, -6);
      values[base] = values[key] === '指定場所' ? (values[base] || '') : values[key];
    });
  ...
  function generateMessage(type, v) {
    if (type === 'pickupConfirm') {
      ...
      return openingClosing(v, paragraphJoin([schedule, v.note]));
    }
    ...
    if (type === 'dinnerWaiting') {
      ...
      if (waiting === '離れた場所で待機し、○時ごろ付近に戻ります。') waiting = v.dinnerReturnTime ? `離れた場所で待機し、${v.dinnerReturnTime}ごろを目安に付近に戻ります。` : '離れた場所で待機し、後ほど付近に戻ります。';
  ...
  function generateFromForm(form) {
    ...
    const item = { id: `${Date.now()}-${Math.random().toString(16).slice(2)}`, createdAt: new Date().toISOString(), templateType: currentType, templateName: MESSAGE_TEMPLATES[currentType].title, message: generateMessage(currentType, values), inputs: values };
  ...
  function showResult(item) {
    ...
    document.getElementById('copyButton').addEventListener('click', () => copyText(document.getElementById('resultText').value));
  ...
  function renderCustomComposer(id, values = {}) {
    ...
    form.addEventListener('submit', (event) => {
      ...
      message = openingClosing(v, message);
      const entry = { id: `${Date.now()}-${Math.random().toString(16).slice(2)}`, createdAt: new Date().toISOString(), templateType: 'custom', customTemplateId: template.id, templateName: template.name, message, inputs: v };
  ...
  function renderPreferences(content) {
    content.innerHTML = `<form id="settingsForm"><section class="settings-group"><h3>文面の初期設定</h3>${selectField('defaultClosing','デフォルト文末',CLOSINGS,data.settings.defaultClosing)}<div class="switch-row"><div><strong>絵文字を使用する</strong><p class="hint">OFFにすると生成文の絵文字を除きます。</p></div><label class="switch"><input id="useEmoji" type="checkbox"${data.settings.useEmoji ? ' checked' : ''} aria-label="絵文字を使用する"><span></span></label></div></section><section class="settings-group"><h3>端末内データ</h3><div class="result-actions"><button id="deleteHistory" class="danger-button" type="button">履歴をすべて削除</button><button id="deletePlaces" class="danger-button" type="button">場所候補をすべて削除</button><button id="resetApp" class="danger-button" type="button">アプリデータを初期化</button></div></section></form>`;
    const form = document.getElementById('settingsForm');
    form.addEventListener('change', () => { data.settings.defaultClosing = form.elements.defaultClosing.value; data.settings.useEmoji = form.elements.useEmoji.checked; saveData(); showToast('設定を保存しました'); });
